# Here we will create a Gradio Application for our GPT Model

# We will first import a few code cells from the "GPT_v1_2" model python notebook
...
# Importing necessary libraries (for Gradio Application)
import gradio as gr
...
# Putting the model into device (GPU)
...
def generate_text(prompt, max_new_tokens=150):
    context = torch.tensor(encode(prompt), dtype=torch.long, device=device)
    generated_chars = decode(model.generate(context.unsqueeze(0), max_new_tokens)[0].tolist())
...
interface = gr.Interface(
    ...
    description="Generate text using the GPT model based on the provided prompt."
)
