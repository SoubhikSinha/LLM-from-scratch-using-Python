...
def get_random_chunk(split):
    filename = "D:/Datasets/train_split.txt" if split == "train" else "D:/Datasets/val_split.txt"
    with open(filename, 'rb') as f:
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
            # Determining the file size and a random position to start reading
            file_size = len(mm)
            start_pos = random.randint(0, (file_size) - block_size * batch_size)

            # Seeking the random position and reading the block of text
            mm.seek(start_pos)
            block = mm.read(block_size*batch_size - 1)

            # Decoding the block to a string, ignoring any invalid byte sequences
...
with open("model_gpt_v1_2.pkl", "wb") as f:
    pickle.dump(model, f)
